...
export default function SignInScreen() {
  ...
  const { signOut: signOutFirebase } = useFirebaseAuth();
  ...
  const handleEmailSignIn = async () => {
    setLocalError('');
    try {
      const credential = await signInWithEmail(emailAddress, password);
      if (!credential.user.emailVerified) {
        await sendVerificationEmail();
        await signOutFirebase();
        setLocalError('Please verify your email first. We sent you a fresh verification link.');
        return;
      }
      goHome();
    } catch (error) {
